...
with open('last_tick.txt') as f:
    last_tick = ast.literal_eval(f.read())
...
def callback(event):
    ...
    tickets = []
    for ticket_id in range(start_at, end_at):
        response = session.get(url + str(ticket_id))
        if response.status_code != 200:
            print('Error with status code {}'.format(response.status_code), ticket_id)
            continue
        ...
        if 'zopim_chat_ended' in ticket['tags']:
            ...
            ctext, atext = not_chat_text(ticket)
        if ctext and atext:
            ticketdict['id'] = ticket['id']
            ticketdict['created_at'] = ticket['created_at']
            ticketdict['tags'] = ticket['tags']
            ticketdict['text_customer'] = ctext
            ticketdict['text_agent'] = atext
